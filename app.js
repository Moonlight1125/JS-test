//parseInt()文字列を整数型に変換
//isNaN()NaNかどうか調べる
//inputのなかが数字以外,負の数,9より大きい場合は一つだけ追加する
const number = document.getElementById("number");
const genButton = document.getElementById("genButton");
const loremText = document.getElementById("dummyText")
...
genButton.addEventListener("click",function(){
    ...
    let atoi = parseInt(value);//valueを整数型にする

    if(isNaN(value)||value<0||value>9){
        let randomNum = Math.floor(Math.random()*article.length)
        loremText.innerHTML =  `<p class="text">${article[randomNum]}</p>`
    }else{
        //slice? 例えば、4を入力したらそれ以降の5個はきりとるようにする。
        let slicedText = article.slice(0,atoi);
        let text = slicedText.map(element=>{
